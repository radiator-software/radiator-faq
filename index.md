...
In general the best practices regarding the timeouts, retries and keepalive settings on the Radiator configuration are dependant of the use case. For example, if database is used for authentication and/or accounting, the Timeout and FailureBackoffTime attributes depend on for example if the database is on the same or different server than the Radiator and if the network between database and Radiator is suitably fast. The same applies to other services that the Radiator may use, like Active Directory and various logging systems.
